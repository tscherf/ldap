...
# 7. Generate the server certificate:
    ...
    echo "Generating server certificate for Red Hat Directory Server on host $myhost"
    ...
    certutil -S $prefixarg -n "Server-Cert" -s "cn=$myhost,ou=Red Hat Directory Server" -c "CA certificate" -t "u,u,u" -m 1001 -v 120 -d $secdir -z $secdir/noise.txt -f $secdir/pwdfile.txt
fi

if test -n "$needASCert" ; then
# Generate the admin server certificate
    echo Creating the admin server certificate
    certutil -S $prefixarg -n "server-cert" -s "cn=$myhost,ou=Red Hat Administration Server" -c "CA certificate" -t "u,u,u" -m 1002 -v 120 -d $secdir -z $secdir/noise.txt -f $secdir/pwdfile.txt
